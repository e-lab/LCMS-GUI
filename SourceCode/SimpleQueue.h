...
// Berin Martini 2012-02-18
// Add ReceiveNoWait member-function which does not wait if queue is empty.
...
enum wxMessageQueueError {
	...
	wxMSGQUEUE_EMTPY         // queue was empty
};
...
public:
	...
	// Same as Receive() but will not wait if the queue is empty.
	wxMessageQueueError ReceiveNoWait (T& msg) {
		wxCHECK (IsOk(), wxMSGQUEUE_MISC_ERROR);
...
private:
	...
	SimpleQueue (const SimpleQueue<T>& rhs);
	SimpleQueue<T>& operator= (const SimpleQueue<T>& rhs);
